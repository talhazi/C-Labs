...
void printFile(int output,linux_dirent* file,int debug,int prefixMode,char type){
...
int main (int argc , char* argv[], char* envp[]){
  ...
  while(i<dirLen){
    ...
    if(prefixMode && strncmp(prefix,file->name,1)==0)
      printFile(output,file,debug,prefixMode,type);
    else if(!prefixMode)
      printFile(output,file,debug,prefixMode,type);
    i=i+file->len;
  }
